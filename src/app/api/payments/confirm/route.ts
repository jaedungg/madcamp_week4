  ...
  try {
    ...
    if (TOSS_SECRET_KEY === 'test_sk_placeholder') {
      ...
      paymentOrder = {
        ...
        status: 'pending',
        plan_type: 'premium',
        toss_order_name: '프리미엄 플랜 구독'
      };
    } else {
      // 실제 데이터베이스에서 결제 주문 조회
    ...
    // 개발 환경에서는 실제 TossPayments API를 호출하지 않고 임시 응답 사용
    let tossData;
    let tossResponse = { ok: true };
    ...
    if (TOSS_SECRET_KEY === 'test_sk_placeholder') {
      ...
      // 실제 TossPayments API 호출
      const apiResponse = await fetch(`${TOSS_API_BASE_URL}/payments/confirm`, {
        method: 'POST',
        headers: {
      ...
      tossData = await apiResponse.json();
      tossResponse = apiResponse;
    ...
    // 개발 환경에서는 데이터베이스 업데이트 건너뛰기
    if (TOSS_SECRET_KEY !== 'test_sk_placeholder') {
      // 실제 환경에서만 데이터베이스 트랜잭션 실행
